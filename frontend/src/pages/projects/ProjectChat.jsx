...
export default function ProjectChat() {
  ...
  async function send() {
    ...
    try {
      ...
      const res = await fetch(base + '/chat?stream=1', {
        ...
        body: JSON.stringify({ message: userText, projectId: id, conversationId: convId || undefined }), signal: controller.signal
      })
  ...
  return (
    <AppLayout title={`Chatting in ${project.name} context`}>
      <div className="card" style={{ height: 'calc(100vh - 200px)', background: '#fff', padding: 0, display: 'flex', flexDirection: 'column' }}>
